...
from music21 import roman, converter, key, pitch
...
class HarmonyEvent:
    def __init__(self, beat: float, rn: roman.RomanNumeral):
        ...
        self.third = PREFERRED_PITCH_CLASSES[rn.third.pitchClass] if rn.third else 'N'
        ...
        self.local_key = self._standardize_key(rn.key.tonicPitchNameWithCase)

    def _standardize_key(self, key: str) -> str:
        is_upper = key.isupper()
        pitch_class = pitch.Pitch(key).pitchClass
        if is_upper:
            return KEY_PITCH_DICT[pitch_class].upper()
        else:
            return KEY_PITCH_DICT[pitch_class].lower()
...
def parse_dcml(path: Path) -> Tuple[List[HarmonyEvent], Tuple[int, int], str]:
    ...
    global_key = key.Key(df["globalkey"].iloc[0])
    
    events: List[HarmonyEvent] = []
    try:
        for _, row in df.iterrows():
            if pd.isna(row["chord"]) or row["numeral"] == "@none" or pd.isna(row["quarterbeats"]):
                continue
            beat = float(Fraction(row["quarterbeats"]))
            local_key = key.Key(roman.RomanNumeral(row["localkey"], global_key).root().name)
            figure_string = row["numeral"].replace("#", "")
            if not pd.isna(row["form"]):
                figure_string += row["form"].replace("%", "ø")
            if not pd.isna(row["figbass"]):
                figure_string += str(int(float(row["figbass"])))
            if not pd.isna(row["changes"]):
                figure_string += \
                    ''.join(f'[add{m}]' for m in re.findall(r'[#b]?(?:11|13|[24679])',
                                                    row["changes"] 
                                                    if isinstance(row["changes"], str) 
                                                    else str(int(float(row["changes"])))))
            if not pd.isna(row["relativeroot"]):
                figure_string += "/"+ row["relativeroot"].split("/")[-1]

            events.append(
                HarmonyEvent(beat, roman.RomanNumeral(figure_string, local_key))
            )
        events.sort(key=lambda e: e.abs_beat)
        return events
    except:
