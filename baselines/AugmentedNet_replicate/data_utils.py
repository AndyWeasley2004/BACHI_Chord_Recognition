...
def get_pianoroll_and_labels(
    ...
    try:
        ...
        # Sort and shift so that the first note onset becomes 0
        notes_data.sort(key=lambda x: x[1])
        first_onset = notes_data[0][1]
        notes_data = [(m, s - first_onset, e - first_onset) for (m, s, e) in notes_data]

        # Canvas length – use the last note's offset as the end time
        ...
        if len(labels) > num_label_frames:
            ...
            if len(labels) > 0:
                ...
                labels = np.array([["None", "None", "None"]] * pad_len, dtype=object)
