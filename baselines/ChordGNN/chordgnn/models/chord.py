import torch
import copy
from chordgnn.models.core import *
...
def coalesce_index(index: torch.Tensor, m: int, n: int) -> torch.Tensor:
    if index.numel() == 0:
        return index
    row = index[0].long()
    col = index[1].long()
...
class OnsetEdgePooling(nn.Module):
    ...
    def __merge_edges__(self, x, edge_index, batch, edge_score):
        ...
        N = new_x.size(0)
        new_edge_index = coalesce_index(cluster[edge_index], N, N)
...
class ChordPrediction(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        try:
            ...
            # Aggregate chord accuracy across available tasks (all-correct across tasks at each onset)
            task_acc_bools = []
            for task_name in self.tasks.keys():
                if task_name in batch_pred and task_name in batch_labels:
                    task_acc_bools.append((batch_pred[task_name].argmax(dim=1) == batch_labels[task_name]).unsqueeze(0))
            if len(task_acc_bools) > 0:
                acc_all = (torch.cat(task_acc_bools, dim=0).sum(0) == len(task_acc_bools)).float().mean()
                self.log('Train Chord Acc', acc_all.item(), on_step=False, on_epoch=True, prog_bar=True, batch_size=batch_size)
            self.log("global_step", self.global_step, on_step=True, prog_bar=False, batch_size=batch_size)
            if not self.use_rotograd:
                return loss["total"]
        except Exception as e:
            print(f"[WARN] Skipping batch due to runtime error: {e}")
            zero = torch.zeros((), device=batch_inputs.device, requires_grad=True)
            return zero
        ...
        try:
            ...
            for task_name in self.tasks.keys():
                if task_name in batch_pred and task_name in batch_labels:
                    task_acc_bools.append((batch_pred[task_name].argmax(dim=1) == batch_labels[task_name]).unsqueeze(0))
            if len(task_acc_bools) > 0:
                acc_all = (torch.cat(task_acc_bools, dim=0).sum(0) == len(task_acc_bools)).float()
                # Use a consistent metric name for checkpointing
                self.log('Val Chord Acc', acc_all.mean().item(), on_step=False, on_epoch=True, prog_bar=True, batch_size=len(acc_all))
        except Exception as e:
            print(f"[WARN] Skipping val batch due to runtime error: {e}")
    ...
    def test_step(self, batch, batch_idx):
        ...
        try:
            ...
            print(f"[WARN] Skipping test batch due to runtime error: {e}")
            
        # Build per-task CSR dict for available tasks
        accs = {}
        if 'acc_all' in locals():
            accs["Chord Acc"] = acc_all.detach().cpu()
        for task_name in self.tasks.keys():
            if task_name in batch_pred and task_name in batch_labels:
                accs[task_name.capitalize()] = (batch_pred[task_name].argmax(dim=1) == batch_labels[task_name]).detach().cpu().squeeze().float()

        acc_ts = self.acc_compute_time_step(accs, batch_labels["onset"].detach().cpu())
        if 'acc_all' in locals():
            self.log('Test Chord Acc (Onset)', acc_all.mean().item(), on_step=False, on_epoch=True, prog_bar=True, batch_size=1)
        if isinstance(acc_ts, dict):
            for k, v in acc_ts.items():
                self.log(f'Test {k} CSR', v.mean().item(), on_step=False, on_epoch=True, prog_bar=True, batch_size=1)
            # Also log average time-step accuracy across tasks
            try:
                avg_ts = sum([v.mean().item() for v in acc_ts.values()]) / len(acc_ts)
                self.log('Test Time Step Accuracy', avg_ts, on_step=False, on_epoch=True, prog_bar=True, batch_size=1)
            except Exception:
                pass
        else:
            self.log('Test Time Step Accuracy', acc_ts.mean().item(), on_step=False, on_epoch=True, prog_bar=True, batch_size=1)
...
class PostChordPrediction(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        onset_edges = edges[:, edge_type == 0]
        onset_idx = unique_onsets(onset_divs)
        # Determine sequence length dimension dynamically from any available task
        any_task = next(iter(batch_labels.keys()))
        b_size = batch_labels[any_task].shape[-1]
        with torch.no_grad():
