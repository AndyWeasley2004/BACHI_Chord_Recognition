...
def get_download_dir():
    ...
    """
    default_dir = os.path.join(os.sep, 'graft1', 'datasets')
    dirname = os.environ.get('chordgnn_DOWNLOAD_DIR', default_dir)
...
class chordgnnDataset(object):
    """The basic chordgnn Dataset for creating various datasets.
    This class defines a basic template class for chordgnn Dataset.
    ...
    def __init__(self, name, features="all", url=None, raw_dir=None, save_dir=None,
                 hash_key=(), force_reload=False, verbose=False, skip_processing=False):
        self._name = name
        self._url = url
        ...
        # if no dir is provided, the default chordgnn download dir is used.
        if raw_dir is None:
    ...
    def save(self):
        ...
        It is recommended to use ``chordgnn.graphs.save()``
        to save chordgnn score graph into files and use
        ``chordgnn.utils.save_info`` to save extra
    ...
    def load(self):
        ...
        It is recommended to use ``chordgnn.utils.load_graph_from_part``
        to load chordgnn Score graph from files and use
        ``chordgnn.utils.load_info`` to load extra information
    ...
    def _load(self):
        r"""Entry point from __init__ to load the dataset.
            if the cache exists:
                Load the dataset from saved chordgnn score graph and information files.
        ...
        """
        # If skipping processing is requested, try to load directly from existing cache
        if getattr(self, "_skip_processing", False):
            if os.path.isdir(self.save_path) and os.listdir(self.save_path):
                self.load()
                if self.verbose:
                    print('Loaded preprocessed data from: {}'.format(self.save_path))
                return
            else:
                raise RuntimeError("skip_processing is set but no preprocessed graphs were found at {}".format(self.save_path))
...
class BuiltinDataset(chordgnnDataset):
    """The Basic chordgnn Builtin Dataset.
