...
class BySequenceLengthSampler(Sampler):
    ...
    def __init__(self, data_source,
        ...
        for i, ex in enumerate(data_source):
            # Our dataset items are lists: [x, edge_index, edge_type, y, onset_divs, name]
            # Use length of y (time steps). Fallback to x rows or onset_divs length.
            seq_len = None
            try:
                if isinstance(ex, (list, tuple)):
                    if len(ex) >= 4 and hasattr(ex[3], 'shape'):
                        seq_len = int(ex[3].shape[0])
                    elif len(ex) >= 1 and hasattr(ex[0], 'shape'):
                        seq_len = int(ex[0].shape[0])
                    elif len(ex) >= 5:
                        # onset_divs
                        od = ex[4]
                        seq_len = int(od.shape[0]) if hasattr(od, 'shape') else int(len(od))
                elif isinstance(ex, dict):
                    if 'y' in ex and hasattr(ex['y'], 'shape'):
                        seq_len = int(ex['y'].shape[0])
                    elif 'x' in ex and hasattr(ex['x'], 'shape'):
                        seq_len = int(ex['x'].shape[0])
            except Exception:
                seq_len = None
            if seq_len is None:
                seq_len = 1
            ind_n_len.append((i, seq_len))
