...
class AugmentedGraphDatamodule(LightningDataModule):
    def __init__(self, batch_size=1, num_workers=4, force_reload=False, include_synth=False, num_tasks=11, collection="all", version="v1.0.0", skip_processing=False):
        ...
        if version == "v1.0.0":
            data_source = AugmentedNetChordGraphDataset(
                ...
                include_synth=include_synth, num_tasks=num_tasks, collection=collection, skip_processing=skip_processing
            )
        elif version == "latest":
            data_source = Augmented2022ChordGraphDataset(
                force_reload=self.force_reload, nprocs=self.num_workers,
                include_synth=include_synth, num_tasks=num_tasks, collection=collection, skip_processing=skip_processing)
        elif version == "pop909":
            # Force 3-task setup for root/quality/bass
            data_source = Pop909ChordGraphDataset(
                force_reload=self.force_reload, nprocs=self.num_workers,
                include_synth=False, num_tasks=3, collection=collection, skip_processing=skip_processing)
        else:
    ...
    def setup(self, stage=None):
        ...
        test_idx = [
            ...
            if self.datasets[self.datasets_map[i][0]].graphs[
                   self.datasets_map[i][1]].collection in ["test", "validation"]
        ]
        ...
        print(
            f"Train size :{len(self.dataset_train)}, Test size :{len(self.dataset_test)}"
        )
    ...
    def collate_fn(self, batch):
        ...
        # Label column order aligns with dataset tasks:
        # v1.0.0/latest use available_representations order; pop909 uses fixed [root, quality, bass, onset]
        if self.version == "pop909":
            ordered_tasks = ["root", "quality", "bass"]
        elif self.version == "v1.0.0":
            from chordgnn.utils.chord_representations import available_representations
            ordered_tasks = list(available_representations.keys())
        else:
            from chordgnn.utils.chord_representations_latest import available_representations
            ordered_tasks = list(available_representations.keys())
        batch_label = {task: batch_labels[:, i].squeeze().long() for i, task in enumerate(ordered_tasks)}
    ...
    def collate_train_fn(self, examples):
        ...
        if self.version == "pop909":
            ...
            from chordgnn.utils.chord_representations_latest import available_representations
            ordered_tasks = list(available_representations.keys())
        batch_label = {task: y[:, :, i].squeeze().long() for i, task in enumerate(ordered_tasks)}
        batch_label["onset"] = y[:, :, -1]
    ...
    def test_dataloader(self):
        return torch.utils.data.DataLoader(
            self.dataset_test, batch_size=1, num_workers=6, collate_fn=self.collate_fn,
            pin_memory=True, persistent_workers=True
        )

    def predict_dataloader(self):
        return torch.utils.data.DataLoader(
            self.dataset_test, batch_size=self.batch_size, num_workers=6, collate_fn=self.collate_fn,
            pin_memory=True, persistent_workers=True
        )
