...
csv_logger = CSVLogger(save_dir="./logs", name="chord_rec", version=name)
...
datamodule = st.data.AugmentedGraphDatamodule(
    ...
    skip_processing=args.skip_processing, force_reload=args.force_reload)
...
if not args.skip_processing:
    if args.data_version == "v1.0.0":
        ...
        # pop909 uses the same representation definitions, but only subset is used
        from chordgnn.utils.chord_representations import available_representations as _avail_repr
        from chordgnn.utils.chord_representations import normalize_pitch_to_sharp, normalize_key_to_sharp
    # Raw vocab (full class lists); inversion not used for pop909
