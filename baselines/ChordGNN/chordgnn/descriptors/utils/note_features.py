import numpy as np
from chordgnn.utils import chord_to_intervalVector
...
def get_chord_analysis_features(part, one_hot=False) -> Tuple[np.ndarray, List]:
    ...
    if one_hot:
        ...
        pitch_names = ["pitch"]
    # Guard against zero or invalid ts_beats to avoid inf/NaN
    ts_beats_safe = note_array["ts_beats"].astype(float)
    ts_beats_safe[ts_beats_safe == 0] = 1.0
    duration_feature = np.expand_dims(1 - np.tanh(note_array["duration_beat"].astype(float) / ts_beats_safe), 1)
    dur_names = ["bar_exp_duration"]
    ...
    metrical_features[:, 0] = np.remainder(note_array["onset_beat"], ts_beats_safe) == 0
    # is integer beat
    metrical_features[:, 1] = np.remainder(note_array["onset_beat"], 1) == 0
    # is half the ts_beats if ts_beats is even
    half_ts = ts_beats_safe / 2.0
    metrical_features[:, 2] = np.remainder(note_array["onset_beat"], half_ts) == 0
    ...
    metrical_features[:, 3] = 1 - np.tanh(time_until_next_onset / ts_beats_safe)
    ca = np.zeros((len(note_array), len(NOTE_FEATURES)))
    for i, n in enumerate(note_array):
        n_onset = note_array[note_array["onset_beat"] == n["onset_beat"]]
        n_dur = note_array[np.where((note_array["onset_beat"] < n["onset_beat"]) & (
