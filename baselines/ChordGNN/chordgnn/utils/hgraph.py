...
class HeteroScoreGraph(object):
    ...
    def save(self, save_dir):
        ...
        arrays = {
            "x": self.x.detach().cpu().numpy() if isinstance(self.x, torch.Tensor) else np.asarray(self.x),
            "edge_index": torch.cat((self.edge_index, self.edge_type.unsqueeze(0)), dim=0).detach().cpu().numpy()
        }
        if isinstance(self.y, torch.Tensor):
            arrays["y"] = self.y.detach().cpu().numpy()
        if isinstance(self.edge_weights, torch.Tensor):
            arrays["edge_weights"] = self.edge_weights.detach().cpu().numpy()
        if isinstance(self.note_array, np.ndarray):
            arrays["note_array"] = self.note_array
        object_properties = dict(vars(self))
        for k in ["x", "edge_index", "edge_type", "edge_weights", "y", "note_array"]:
            if k in object_properties:
                del object_properties[k]
        arrays["graph_info"] = np.array(object_properties, dtype=object)
        np.savez_compressed(os.path.join(save_dir, save_name, "graph.npz"), **arrays)
...
def load_score_hgraph(load_dir, name=None):
    ...
    npz_path = os.path.join(path, "graph.npz")
    if not os.path.exists(npz_path):
        raise ValueError("Missing 'graph.npz' in the provided directory.")
    data = np.load(npz_path, allow_pickle=True)
    x = data["x"]
    edge_index = data["edge_index"]
    y = data["y"] if "y" in data.files else None
    edge_weights = data["edge_weights"] if "edge_weights" in data.files else None
    note_array = data["note_array"] if "note_array" in data.files else None
    graph_info = data["graph_info"].item() if "graph_info" in data.files else {}
    resolved_name = name if name else os.path.basename(path)
    hg = HeteroScoreGraph(note_features=x, edges=edge_index, name=resolved_name, labels=y, edge_weights=edge_weights, note_array=note_array)
