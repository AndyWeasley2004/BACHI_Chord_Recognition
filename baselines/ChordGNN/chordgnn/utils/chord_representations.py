...
import numpy as np
import pandas as pd
from chordgnn.utils.globals import *
...
_transposePitchCanonical = {}
_transposeKeyCanonical = {}
...
# Canonical sharp-only spelling set for pitch-class names
SHARP_SPELLINGS = (
...
def create_data(filtered_df, time_signature, interval="P1"):
    ...
    y = np.stack(
        ...
        axis=1)
    def _safe_eval_literal(x):
        try:
            return ast.literal_eval(x)
        except Exception:
            return None
    def _safe_parse_pitch_token(pitch_str):
        try:
            pitch_t = TransposePitch(pitch_str, interval)
            parts = re.findall(r'[A-Za-z]+|[#-]+|\d+', pitch_t)
            if len(parts) == 3:
                step, alter_str, octave_str = parts
            elif len(parts) == 2:
                step, octave_str = parts
                alter_str = ""
            else:
                return None
            if alter_str not in ALTER:
                return None
            octave = int(octave_str)
            alter_val = ALTER[alter_str]
            mp = partitura.utils.pitch_spelling_to_midi_pitch(step, alter_val, octave)
            return step, alter_val, octave, mp
        except Exception:
            return None

    for i, row in filtered_df.iterrows():
        pitches = _safe_eval_literal(row["s_notes"]) if isinstance(row["s_notes"], str) else row["s_notes"]
        if pitches is None:
            continue
        # is_onset = eval(row["s_isOnset"])
        for j, pitch in enumerate(pitches):
            parsed = _safe_parse_pitch_token(pitch)
            if parsed is None:
                continue
            step, alter, octave, mp = parsed
...
def create_data_pop(filtered_df, time_signature, interval="P1"):
    """Create note array and labels for Pop909-style TSVs with only root, quality, bass.

    Returns (X, y) where y columns are [root, quality, bass, onset].
    """
    note_array = list()
    onset = np.expand_dims(filtered_df["j_offset"].to_numpy(), axis=1)
    root = encode_one_hot(filtered_df, ChordRoot35, transposition=interval)
    quality = encode_one_hot(filtered_df, ChordQuality15, transposition=interval)
    bass = encode_one_hot(filtered_df, Bass35, transposition=interval)
    y = np.stack((root, quality, bass, onset), axis=1)
...
def time_divided_tsv_to_note_array(time_divided_tsv_path, transpose=False, version="v1.0.0"):
    ...
    # Prepare fields depending on version
    if version == "v1.0.0":
        ...
        createfunction = create_data
    elif version == "latest":
        createfunction = create_data_latest
    elif version == "pop909":
        # Pop909 only uses root, quality, and bass; no extra field conversions needed
        createfunction = create_data_pop
    else:
        raise ValueError(f"Unknown dataset version: {version}")
    if transpose:
        if version == "v1.0.0":
            transpositions = _getTranspositions(filtered_df)
        elif version == "latest":
            transpositions = _getTranspositions_latest(filtered_df)
        else:
            # For Pop909, enable generic 11 interval transpositions (exclude unison)
            try:
                transpositions = [iv for iv in INTERVALCLASSES if iv != "P1"]
            except Exception:
                transpositions = ["m2","M2","m3","M3","P4","TT","P5","m6","M6","m7","M7"]
...
def normalize_key_to_sharp(key: str) -> str:
    """Return sharp-only tonic name preserving case for mode (lowercase for minor)."""
    try:
        tonic_name = m21Key(key).tonicPitchNameWithCase
        is_lower = tonic_name[0].islower()
        pc = m21Pitch(tonic_name).pitchClass
        name = _pc_to_sharp_name(pc)
        return name.lower() if is_lower else name
    except Exception:
        return key


def TransposeKeyCanonical(key, interval):
    """Transpose key then canonicalize tonic to sharp-only spelling, preserving case."""
    duple = (key, interval)
    if duple in _transposeKeyCanonical:
        return _transposeKeyCanonical[duple]
    try:
        keyObj = m21Key(key)
        transposed_name = keyObj.transpose(interval).tonicPitchNameWithCase
        is_lower = transposed_name[0].islower()
        pc = m21Pitch(transposed_name).pitchClass
        name = _pc_to_sharp_name(pc)
        out = name.lower() if is_lower else name
    except Exception:
        out = key
    _transposeKeyCanonical[duple] = out
    return out
...
def _pc_to_sharp_name(pc: int) -> str:
    mapping = {
        0: "C", 1: "C#", 2: "D", 3: "D#", 4: "E", 5: "F",
        6: "F#", 7: "G", 8: "G#", 9: "A", 10: "A#", 11: "B"
    }
    return mapping[int(pc) % 12]


def normalize_pitch_to_sharp(pitch: str) -> str:
    try:
        pc = m21Pitch(pitch).pitchClass
        return _pc_to_sharp_name(pc)
    except Exception:
        return pitch


def TransposePitchCanonical(pitch, interval):
    """Transpose pitch then canonicalize to sharp-only spelling without octave (12-name set)."""
    duple = (pitch, interval)
    if duple in _transposePitchCanonical:
        return _transposePitchCanonical[duple]
    try:
        pitchObj = m21Pitch(pitch)
        transposed = pitchObj.transpose(interval)
        name = _pc_to_sharp_name(transposed.pitchClass)
    except Exception:
        name = pitch
    _transposePitchCanonical[duple] = name
    return name
...
class Bass35(OutputRepresentation):
    classList = SPELLINGS
    dfFeature = "a_bass"
    transpositionFn = staticmethod(TransposePitchCanonical)

    @classmethod
    def decode(cls, array):
        tokens = [cls.classList[index] for index in array.reshape(-1)]
        return [normalize_pitch_to_sharp(t) for t in tokens]
...
class LocalKey35(OutputRepresentation):
    ...
    transpositionFn = staticmethod(TransposeKeyCanonical)

    @classmethod
    def decode(cls, array):
        tokens = [cls.classList[index] for index in array.reshape(-1)]
        return [normalize_key_to_sharp(t) for t in tokens]
